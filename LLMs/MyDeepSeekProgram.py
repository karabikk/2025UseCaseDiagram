...
df = pd.read_csv("law_requirements.csv")
...
chain1 = response.choices[0].message.content.strip()
print(chain1)
...
chain2 = response2.choices[0].message.content.strip()
print(chain2)
...
chain3 = response3.choices[0].message.content.strip()
print(chain3)
...
chain4 = response4.choices[0].message.content.strip()
print(chain4)

response5 = client.chat.completions.create(
    model="deepseek-chat",
    messages=[
                {"role": "user", "content":f" You are given three tasks. Your first task is to convert the actor use case association arrows into plain, non-directional lines. Your second task is to find any <<extend>> relationship and rewrite it using a dotted arrow with UP directional modifier using this format: X .up.> Y : <<extend>> Your third task is find any <<include>> relationship and rewrite it using a dotted arrow with DOWN directional modifier using this format: X .down.> Y :<<include>> {chain4}"}
    ]
)
chain5 = response5.choices[0].message.content.strip()
print(chain5)